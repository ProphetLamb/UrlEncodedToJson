...
namespace UrlEncodedToJson.Buffers;

/// <summary>
/// `System.Buffers.ArrayBufferWriter`, but allocates from `ArrayPool{T}.Shared`.
/// </summary>
internal sealed class PooledBufferWriter<T> : IBufferWriter<T>, IDisposable
{
    ...
    private void CheckAndResizeBuffer(int sizeHint)
    {
        if (sizeHint < 0)
        {
            throw new ArgumentException("Value must be positive or zero", nameof(sizeHint));
        }
...
#pragma warning disable CA2201
        throw new OutOfMemoryException($"BufferMaximumSizeExceeded capacity={capacity}");
